...
fn workflow_run_with_inputs_prompt(arguments: Option<&Map<String, Value>>) -> Result<GetPromptResult, rmcp::model::ErrorData> {
    ...
    Ok(GetPromptResult {
        description: Some("Prepare workflow input values for workflow_resolve_inputs or workflow_run.".to_string()),
        messages: vec![PromptMessage::new_text(
            PromptMessageRole::User,
            format!(
