...
use crate::log_persistence::PersistentLogWorker;
...
pub struct App<'a> {
    ...
    /// Optional background worker for persistent TUI diagnostics logs.
    persistent_log_worker: Option<PersistentLogWorker>,
    /// Count of persistent log entries dropped due to backpressure.
    persistent_log_drop_count: u64,
...
impl App<'_> {
    ...
    pub fn new(registry: Arc<Mutex<CommandRegistry>>, plugin_engine: Arc<PluginEngine>) -> Self {
        ...
        let mut app = Self {
            ...
            persistent_log_worker: None,
            persistent_log_drop_count: 0,
        };
        ...
        app.initialize_persistent_log_writer();
    ...
    pub fn append_log_message_with_level(&mut self, level: Option<LogLevel>, message: impl Into<String>) {
        ...
        let original_entry_count = self.logs.rich_entries.len();
        self.logs.append_text_entry_with_level(level, text);
        self.persist_entries_since(original_entry_count);
    }
    ...
    /// Processes an execution outcome and persists any log entries generated by
    /// that outcome.
    pub fn process_general_execution_result(&mut self, execution_outcome: &oatty_types::ExecOutcome) {
        let original_entry_count = self.logs.rich_entries.len();
        self.logs.process_general_execution_result(execution_outcome);
        self.persist_entries_since(original_entry_count);
    }

    fn initialize_persistent_log_writer(&mut self) {
        match PersistentLogWorker::from_environment() {
            Ok(worker) => {
                tracing::info!(path = %worker.path().display(), "TUI persistent logging enabled.");
                self.persistent_log_worker = Some(worker);
            }
            Err(error) => {
                warn!(
                    error = %error,
                    "Failed to initialize persistent TUI log writer; continuing with in-memory logs only."
                );
                self.persistent_log_worker = None;
            }
        }
    }

    fn persist_entries_since(&mut self, original_entry_count: usize) {
        let Some(worker) = self.persistent_log_worker.as_ref() else {
            return;
        };
        let Some(new_entries) = self.logs.rich_entries.get(original_entry_count..) else {
            return;
        };

        for entry in new_entries {
            match worker.enqueue_entry(entry) {
                Ok(true) => {}
                Ok(false) => {
                    self.persistent_log_drop_count = self.persistent_log_drop_count.saturating_add(1);
                    if self.persistent_log_drop_count.is_multiple_of(128) {
                        warn!(
                            dropped_entries = self.persistent_log_drop_count,
                            path = %worker.path().display(),
                            "Persistent TUI log queue is full; dropping log entries."
                        );
                    }
                }
                Err(error) => {
                    warn!(
                        error = %error,
                        path = %worker.path().display(),
                        "Failed to append persistent TUI log entry."
                    );
                    break;
                }
            }
