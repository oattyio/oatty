...
impl ManualEntryComponent {
    fn render_block(&self, frame: &mut Frame, area: Rect, app: &App) -> Rect {
        let Some(state) = app.workflows.manual_entry_state() else {
            return Rect::default();
        };
        let title = format!("Manual entry: {}", state.label);
        let block = th::block(&*app.ctx.theme, Some(title.as_str()), state.f_input.get());
...
impl Component for ManualEntryComponent {
    ...
    fn render(&mut self, frame: &mut Frame, rect: Rect, app: &mut App) {
        ...
        let layout = ManualEntryLayoutState::from(self.get_preferred_layout(app, inner));
