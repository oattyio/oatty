...
use seekstorm::{
    commit::Commit,
    index::{
        AccessType, FrequentwordType, IndexArc, IndexDocuments, IndexMetaObject, NgramSet, SchemaField, SimilarityType, StemmerType,
        StopwordType, Synonym, TokenizerType, create_index,
    },
};
...
impl Indexer {
    pub async fn start(&mut self) -> Result<(), IndexerError> {
        ...
        let index_commands = async move || -> Result<(), IndexerError> {
            let new_documents = {
                let registry_lock = registry.lock().map_err(|e| IndexerError::Lock(e.to_string()))?;
                registry_lock
                    .commands
                    .iter()
                    .map(|c| {
    ...
    async fn index(&mut self) -> Result<IndexArc, IndexerError> {
        ...
        create_index(&index_path, meta, &schema, &synonyms, 11, false, None)
            .await
