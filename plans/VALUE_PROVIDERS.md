...
- Define a **registry format** for mapping flags → provider IDs. DONE:
  - `CommandSpec.providers: Vec<ProviderBinding>` where each entry is:
    - `kind`: `Flag | Positional`
    - `name`: parameter name (e.g., `app`, `addon`)
    - `provider_id`: provider command id (e.g., `apps:list`)
    - `confidence`: `High | Medium | Low`
  - Generated by `registry-gen` in a second pass over all commands.
...
- Prototype Guided UI integration with async dropdown population. DONE (palette):
  - `RegistryBackedProvider` reads `CommandSpec.providers` and fetches values using shared HTTP helper.
  - Suggestions merge enum values, history, and provider values with caching and fuzzy filtering.
